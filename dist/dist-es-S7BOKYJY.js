...
var resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./dist-es-LKVPQI4Z.js");
...
var resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./sts-S747ONTY.js");
...
var resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./dist-es-ZJXQLKWF.js");
  return fromSSO({
    profile,
...
var resolveWebIdentityCredentials = async (profile, options) => import("./dist-es-YMWTYYMX.js").then(({ fromTokenFile }) => fromTokenFile({
