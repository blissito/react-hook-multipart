import { S3Client, PutBucketCorsCommand, CreateMultipartUploadCommand, UploadPartCommand, CompleteMultipartUploadCommand, DeleteObjectCommand, DeleteObjectsCommand, GetObjectCommand, HeadObjectCommand, PutObjectCommand, ListObjectsV2Command, } from "@aws-sdk/client-s3";
...
export const listObjectsInFolder = (Prefix) => getS3Client().send(new ListObjectsV2Command({ Bucket, Prefix }));
export const completeMultipart = ({ ETags, UploadId, Key, }) => {
    return getS3Client().send(new CompleteMultipartUploadCommand({
        Bucket,
        Key,
        UploadId,
        MultipartUpload: {
...
export const getPutPartUrl = (options) => getSignedUrl(getS3Client(), new UploadPartCommand({
    Bucket,
    Key: options.Key,
...
export const createMultipart = async (fileName, ACL = "private") => {
    ...
    // await setCors();
    const { UploadId } = await getS3Client().send(new CreateMultipartUploadCommand({
        Bucket,
        Key,
        ACL,
    }));
...
export const deleteObject = (Key, bucket) => {
    return getS3Client().send(new DeleteObjectCommand({
...
export const getDeleteFileUrl = async (Key) => getSignedUrl(getS3Client(), new DeleteObjectCommand({
    Bucket,
    Key,
}), { expiresIn: 3600 });
